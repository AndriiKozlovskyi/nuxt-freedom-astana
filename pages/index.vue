<template>
  <div>
    <AppLanguageSelector v-if="!languageSelected" @language-select="handleLanguageSelect" />
    <template v-else>
...
watch(languageSelected, async (selected) => {
  if (!selected) return
  await nextTick() // wait for v-else DOM to mount
  revealAll()
  bindPageEffects()
})
</script>
